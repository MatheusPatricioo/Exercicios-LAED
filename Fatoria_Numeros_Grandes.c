...
int main() {
    int n;
    scanf("%d", &n); // Solicita ao usuário que insira o número para o qual o fatorial será calculado

    if (n < 2) {
        printf("Resultado: 1\n"); // Se 'n' for menor que 2, o fatorial é 1, então imprime o resultado e sai
    } else {
        int result[MAX_SIZE] = {0}; // Inicializa um array para armazenar o resultado do fatorial, preenchido com zeros
...
void show_result(int *result, int size) {
    for (int i = size - 1; i >= 0; i--) { // Loop reverso para imprimir o número armazenado em 'result'
        printf("%d", result[i]); // Imprime cada dígito do número armazenado em 'result'
    }
